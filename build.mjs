...
/* ==========================================================================
   ...
   Uso:  SITE_URL=https://veredictfx.com  node build.mjs
   ========================================================================== */
...
// artículos publicados antes del rebrand) → Veredict FX / veredictfx.com. Así no hay que
// editar cada JSON a mano y cualquier artículo viejo queda coherente al construir.
function fixBrand(v) {
  if (typeof v === 'string') {
    return v
      .replace(/FAROFX/g, 'Veredict FX')
      .replace(/\bFARO\b/g, 'Veredict FX')
      .replace(/farofx\.org/g, 'veredictfx.com');
  }
  if (Array.isArray(v)) return v.map(fixBrand);
  if (v && typeof v === 'object') { const o = {}; for (const k of Object.keys(v)) o[k] = fixBrand(v[k]); return o; }
  return v;
}
...
page('/privacidad/', renderSimplePage({
  ...
  <p><b>Responsable del tratamiento.</b> El responsable del tratamiento de tus datos es el titular de veredictfx.com. Contacto: <a href="mailto:contacto@veredictfx.com" style="color:var(--seal)">contacto@veredictfx.com</a>.</p>
...
writeFileSync(join(DIST, 'ABRIR-PREVIEW-VEREDICTFX.html'), standalone(homeHtml));
