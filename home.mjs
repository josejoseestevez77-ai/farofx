...
import { SITE, esc, computeScore, scoreBreakdown, scoreColor, riskLabel, fmt, AUDIT_LABELS, podium, regCode, EU_UK_CODES } from './helpers.mjs';
  ...
  return brokers
    .map((b) => {
      ...
      return {
        ...
        noEuUk,
        regs,
        audit: Object.keys(AUDIT_LABELS).map((k) => [AUDIT_LABELS[k], bd.pillars[k]]),
  ...
  const jsonld = [
    {
      ...
      '@type': 'Organization',
      name: 'Veredict FX',
      alternateName: ['VEREDICTFX', 'Veredictfx', 'Veredict'],
      url: SITE.url + '/',
    ...
    },
    {
      ...
      '@type': 'WebSite',
      name: 'Veredict FX',
      alternateName: ['Veredictfx', 'Veredict FX opiniones', 'Veredict', 'VEREDICTFX'],
      url: SITE.url + '/',
      inLanguage: 'es-ES',
      sameAs: SITE.sameAs,
